...
bool FactoredModel::initMDPModel(int nfactors){
  ...
  for (int i = 0; i < nfactors; i++){
    if (nModels == 1){
      ...
      cout << "Invalid model type for Factored Model" << endl;
      exit(-1);
    }
...
bool FactoredModel::getStateActionInfo(const std::vector<float> &state, int act, StateActionInfo* retval){
  ...
  if (needConf){
    ...
    float transConf = retval->conf / (float)state.size();
    transConf += retval->termProb;
    if (transConf > 1.0) transConf = 1.0;
    float avgConf = (rConf + transConf + tConf) / 3.0;

    if (MODEL_DEBUG) cout << "orig transconf: " << (retval->conf / (float)state.size()) << ", adjusted: " << transConf << ", t: " << tConf << " r: " << rConf << " avg: " << avgConf << endl;
  ...
  retval->known = (retval->conf > 0.999);
  return true;
